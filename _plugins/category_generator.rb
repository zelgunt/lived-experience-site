...
#
# Included tags :
# - category_list: Outputs a list of categories
#
    ...
    def write_category_indexes
      if self.layouts.key? 'category_index'
        ...
        self.categories.keys.each do |category|
          self.write_category_index(File.join(dir, category.gsub(/ +/, '-').gsub(/_|\P{Word}/, '-').gsub(/-{2,}/, '-').downcase), category)
        end
...
module Jekyll
  class CategoryList < Liquid::Tag
      ...
      begin
        ...
        unless categories.nil?
          ...
          categories.sort_by { |cat, posts| posts.size }
            .reverse()
            .each do |cat, posts|
               s << %(<li><a href="#{context['site']['url']}/#{context['site']['category_dir']}/#{cat.gsub(/ +/, '-').gsub(/_|\P{Word}/, '-').gsub(/-{2,}/, '-').downcase}">#{cat}</a></li>)
            end
        end
      rescue => boom
        p boom
    ...
    Liquid::Template.register_tag('category_list', self)
  end
end
